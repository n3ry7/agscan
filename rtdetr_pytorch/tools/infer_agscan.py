...
def draw(images, labels, boxes, scores, thrh=0.3, output_path=""):
    for i, im in enumerate(images):
        ...
        for j, b in enumerate(box):
            # Convert tensor to list with proper detachment
            if isinstance(b, torch.Tensor):
                b = b.detach().cpu().numpy()  # Add detach()
...
def save_class_crops(image, labels, boxes, scores, class0_dir, class1_dir, thrh=0.3):
    scr = scores[0]
    ...
    for j, b in enumerate(box):
        # Convert tensor with proper detachment
        if isinstance(b, torch.Tensor):
            b = b.detach().cpu().numpy()  # Add detach()
        b = [int(round(x)) for x in b]
        ...
        base_name = f"crop_{b[0]}_{b[1]}_{b[2]}_{b[3]}_{score:.2f}.jpg"
        crop.save(os.path.join(output_dir, base_name))
...
def main(args):
    ...
    class0_dir.mkdir(exist_ok=True)
    class1_dir.mkdir(exist_ok=True)
    ...
    for image_file in image_files:
        ...
        draw([im_pil.copy()], labels, boxes, scores, 0.3, str(output_image_path))
        
        # Save class crops
        save_class_crops(im_pil, labels, boxes, scores, class0_dir, class1_dir, 0.3)
  
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-c', '--config', type=str, required=True)
    parser.add_argument('-r', '--resume', type=str, required=True)
    parser.add_argument('-i', '--input-dir', type=str, required=True, help='Directory containing input images')
    parser.add_argument('-o', '--output-dir', type=str, required=True, help='Directory to save output images and crops')
    parser.add_argument('-s', '--sliced', action='store_true', help='Use sliced inference')
